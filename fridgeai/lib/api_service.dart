...
static Future<Map<String, dynamic>> fetchNutritionalInfo(int recipeId) async {
  return _handleApiError(() async {
    ...
    if (apiKey == null) throw const ApiException('SPOONACULAR_API_KEY not found');
    ...
    final uri = Uri.parse('$spoonacularBaseUrl/recipes/$recipeId/nutritionWidget.json')
    ...
    if (response.statusCode == 200) {
      ...
      throw ApiException('Failed to fetch nutritional info: ${response.body}');
    }
  });
}
  ...
  static Future<List<String>> analyzeImage(File imageFile) async {
    return _handleApiError(() async {
      ...
      if (imageSize > 10 * 1024 * 1024) { // 10MB limit
        throw const ApiException('Image size too large. Please use an image under 10MB.');
      }
